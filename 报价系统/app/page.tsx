...
import { AddModelDialog, AddRepairDialog, AddBrandDialog } from "@/components/add-dialog";
import { BrandManager } from "@/components/brand-manager";
import { processCSV, analyzeErrors, exportToCSV, convertToDbRecords, convertFromDbRecords } from "@/lib/quote-utils";
import type { BrandType, ModelItem, QuoteRecord } from "@/lib/types";
...
export default function QuoteManagement() {
  ...
  useEffect(() => {
    if (dbData && dbData.length > 0 && !hasLoadedFromDb) {
      console.log("[v0] Loading data from database:", dbData.length, "records");
      const converted = convertFromDbRecords(dbData);
      console.log("[v0] Converted to", converted.length, "models");
      setAppData(converted);
      setHasLoadedFromDb(true);
      // DO NOT auto-save here - data is already in the database
  ...
  // Track if we need to save (only after user imports new CSV)
  ...
  const saveToDatabase = async (data: ModelItem[], replaceAll: boolean = false) => {
    ...
    try {
      ...
      console.log("[v0] Saving records to database:", records.length, "records", replaceAll ? "(replacing all)" : "(upsert)");
      ...
      console.log("[v0] Save response:", response.status, result);
